...
class CheckerBoard:
    def __init__(self):
        self.board_dim = 10
        self.board = [
            [(i + j + 1) % 2 for i in range(self.board_dim)]
            for j in range(self.board_dim)
        ]
        self.blacks = np.array(
            [
                [
                    1 if j <= 3 and self.board[i][j] == 0 else 0
                    for i in range(self.board_dim)
                ]
                for j in range(self.board_dim)
            ]
        )
        self.whites = np.array(
            [
                [
                    1 if j >= (self.board_dim - 4) and self.board[i][j] == 0 else 0
                    for i in range(self.board_dim)
                ]
                for j in range(self.board_dim)
            ]
    ...
    def as_tensor(self):
        ...
        total_state = (
            torch.cat([whites_tensor, blacks_tensor], dim=0).to(device).float()
        )
    ...
    def check_valid_move(self, player_board, opponent_board, x, y):
        return (
            ...
            and opponent_board[x][y] == 0
            and player_board[x][y] == 0
        )
    ...
    def action_idx_to_board(action_idx):
        ...
        board = np.zeros((board_dim, board_dim, 2), dtype=np.float32)
        src_board = board[:, :, 0]
        dest_board = board[:, :, 1]
        src_board[i][j] = 1.0
        dest_board[x][y] = 1.0
    ...
    def get_valid_actions(self, player) -> dict:
        ...
        for i in range(self.board_dim):
            for j in range(self.board_dim):
                ...
                simple_move_positions = (
                    [(i - 1, j + 1), (i - 1, j - 1)]
                    if player == "white"
                    else [(i + 1, j + 1), (i + 1, j - 1)]
                )
                simple_move_positions = filter(
                    lambda pos: self.check_valid_move(
                        player_board, opponent_board, pos[0], pos[1]
                    ),
                    simple_move_positions,
                )
                ...
                for capture_position in capture_move_positions:
                    ...
                    if opponent_board[mid_x][mid_y] == 1.0:
                        ...
                        src_board[i][j] = 1.0
                        dest_board[x][y] = 1.0
                        action_flattened_idx = action_base_idx + (
                            x * self.board_dim + y
                        )
    ...
    def execute(self, action: np.ndarray, player):
        ...
        dest_x, dest_y = np.unravel_index(
            dest_board.argmax(), dest_board.shape
        )  # (i, j)
        player_board = self.whites if player == "white" else self.blacks
        opponent_board = self.whites if player == "black" else self.blacks
